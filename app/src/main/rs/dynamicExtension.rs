...
void RS_KERNEL computeLutGray(uchar in, uint32_t x){
    lutGray[x] = truncate(newMinMaxRGB[0].x + (newMinMaxRGB[0].y - newMinMaxRGB[0].x) * (x - minMaxRGB[0].x) / (minMaxRGB[0].y - minMaxRGB[0].x));
}
...
void computeNewMinMaxGray(){
    ...
    minMaxGray.x = middle - factor;
    minMaxGray.y = middle + factor;
}
